                    ...
                    <div id="sellers-tab" class="tab-content">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select name="seller_status" id="seller_status"
                                            ...
                                            <option value="pending" {{ request('seller_status') === 'pending' ? 'selected' : '' }}>Pending Verification</option>
                                        </select>
                                    </div>
                                    <div class="flex items-end">
                                        <div class="space-x-2">
                                            ...
                                            <a href="{{ route('admin.verifications.users.index') }}?active_tab=sellers" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                Clear
                                            </a>
                                        </div>
                                    </div>
                        ...
                        @if($sellersWithDocuments->count() > 0)
                                        ...
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    ...
                                                    <form method="POST" action="{{ route('admin.verifications.users.seller.verify', $user) }}" class="inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" 
                                                    ...
                                                    <form method="POST" action="{{ route('admin.verifications.users.seller.reject', $user) }}" class="inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" 
                            ...
                            <div class="flex items-center justify-between mt-6">
                                ...
                                <div class="mt-4">
                                    {{ $sellersWithDocuments->appends(request()->except('sellers_page'))->appends('active_tab', 'sellers')->links() }}
                                </div>
                            </div>
                            @endif
                        @else
                    ...
                    <div id="buyers-tab" class="tab-content hidden">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select name="buyer_status" id="buyer_status"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2 px-3">
                                            <option value="">All Statuses</option>
                                            <option value="approved" {{ request('buyer_status') === 'approved' ? 'selected' : '' }}>Approved</option>
                                            <option value="pending" {{ request('buyer_status') === 'pending' ? 'selected' : '' }}>Pending Approval</option>
                                        </select>
                                    </div>
                                    <div class="flex items-end">
                                        <div class="space-x-2">
                                            ...
                                            <a href="{{ route('admin.verifications.users.index') }}?active_tab=buyers" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                Clear
                                            </a>
                                        </div>
                                    </div>
                        ...
                        @if($buyers->count() > 0)
                                ...
                                <table class="w-full">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        ...
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    <a href="{{ route('admin.verifications.users.buyer.show', $user) }}" 
                                                       class="text-blue-600 hover:text-blue-800 cursor-pointer" title="View Details">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                    ...
                                                    @if(!$user->is_verified)
                                                    <form method="POST" action="{{ route('admin.verifications.users.buyer.approve', $user) }}" class="inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" 
                                                                class="text-green-600 hover:text-green-800 cursor-pointer"
                                                    ...
                                                    <form method="POST" action="{{ route('admin.verifications.users.buyer.reject', $user) }}" class="inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" 
                                                                class="text-red-600 hover:text-red-800 cursor-pointer"
                            ...
                            <div class="flex items-center justify-between mt-6">
                                ...
                                <div class="mt-4">
                                    {{ $buyers->appends(request()->except('buyers_page'))->appends('active_tab', 'buyers')->links() }}
                                </div>
                            </div>
                            @endif
                        @else
