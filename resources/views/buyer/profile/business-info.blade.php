...
<div class="bg-white rounded-xl border border-gray-200 p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2">
            <label class="block text-sm font-medium text-gray-700 mb-1">Products Interested In</label>
            <div class="text-gray-900">{{ Auth::user()->profile->products_interested ?? 'Not provided' }}</div>
        </div>
        
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Buying Frequency</label>
            <div class="text-gray-900">{{ Auth::user()->profile->buying_frequency ?? 'Not provided' }}</div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="products_interested" id="products_interested" rows="3" 
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ Auth::user()->profile->products_interested }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="buying_frequency" id="buying_frequency" 
                    ...
                    <option value="weekly" {{ Auth::user()->profile->buying_frequency === 'weekly' ? 'selected' : '' }}>Weekly</option>
                    <option value="monthly" {{ Auth::user()->profile->buying_frequency === 'monthly' ? 'selected' : '' }}>Monthly</option>
                    <option value="quarterly" {{ Auth::user()->profile->buying_frequency === 'quarterly' ? 'selected' : '' }}>Quarterly</option>
                    <option value="yearly" {{ Auth::user()->profile->buying_frequency === 'yearly' ? 'selected' : '' }}>Yearly</option>
                    <option value="occasional" {{ Auth::user()->profile->buying_frequency === 'occasional' ? 'selected' : '' }}>Occasional</option>
                </select>
            </div>
